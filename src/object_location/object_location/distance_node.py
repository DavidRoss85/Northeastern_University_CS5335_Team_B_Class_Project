...
class DistanceNode(Node):
    ...
    DEFAULT_GAZEBO_DEPTH_MIN = .001
    DEFAULT_GAZEBO_DEPTH_MAX = 65
    ...
    DEFAULT_CAMERA_FOV = 78.0
    DEFAULT_GAZEBO_CAMERA_FOV = 78.24 
    ...
    def __init__(self):
        ...
        self.__camera_angle_width = self.DEFAULT_CAMERA_FOV if not self.__using_gazebo else self.DEFAULT_GAZEBO_CAMERA_FOV        # Horizontal field of view of the camera (degrees)
        ...
        self.__depth_min = self.DEFAULT_DEPTH_MIN if not self.__using_gazebo else self.DEFAULT_GAZEBO_DEPTH_MIN
        self.__depth_max = self.DEFAULT_DEPTH_MAX if not self.__using_gazebo else self.DEFAULT_GAZEBO_DEPTH_MAX
    ...
    def __process_detections(self, message: RSyncDetectionList):
        ...
        for item in detection_list:
            ...
            if relative_location.distance > 0:
                print(f"Item: {item.name}, Distance: {relative_location.distance:.2f} m, Yaw: {relative_location.relative_yaw:.2f}°")
        ...
        if self.__show_image:
            ...
            #         continue
            #     closest_point_on_x = min(closest_point_on_x, depth_map[yc,n])
            disp_vis = cv2.normalize(depth_map, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
            cv2.namedWindow("Depth Image", cv2.WINDOW_NORMAL)
            cv2.imshow("Depth Image", disp_vis)
            cv2.waitKey(1)
